...
facet_grid(Country ~ .) +  scale_alpha_manual("Risk Group", labels = function(high_risk) c("Low","High")[high_risk+1], values = c(`FALSE`=0.5,`TRUE`=0.7)) +
  scale_x_continuous("Number of Infections") + scale_fill_discrete() + scale_color_discrete() + scale_linetype_manual("Measure", values=c(`Lower Quartile`="dashed",Median="solid",`Upper Quartile`="dotted")) +

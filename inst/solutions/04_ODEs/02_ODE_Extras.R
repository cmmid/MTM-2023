...
# Answer:
# The period is longer than the vector of times, so a full cycle is not
# completed within our model solution
...
# For example, assume an SI model with births represents infection in a
# livestock population. If more than half of the target herd size becomes
# infected, the infected animals are culled at a daily rate of 0.5.
...
root <- function(times, state, parms) {
  ## Extract relevant variables / parameters
  I <- state["I"]
  K <- parms["K"]
...
# Answer: more animals are added to the susceptible state, but the same cycle
# occurs, infection is never eradicated.

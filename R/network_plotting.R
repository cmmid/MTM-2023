...
#'
#' @return a [data.table], with columns `V2`, `V1` (vertex ids),
#' `eid` (edge ids), layout data (`x1`, `y1`, `x2`, `y2`), and then optionally
#' edge attributes (various, corresponding to the [igraph::edge_attr()] of `ig`)
#' and vertex attributes (various, corresponding to the [igraph::vertex_attr()]
#' of `ig`, prepended with `V1` and `V2` to identify associated vertex).
#'
...
network_vertex_data <- function(dt) {
  ...
  v2 <- unique(
    dt[, .SD, .SDcols = patterns("^V2|x2|y2|time")]
  )[!(V2 %in% v1$vid)]
...
scale_color_network <- rejig(
  ...
  guide = "none", values = c(SIR_colors, c(active = "red", inactive = "grey"))
)
...
geom_edge <- rejig(
  geom_segment,
  mapping = aes(
    x = x1, y = y1, xend = x2, yend = y2, color = state, group = eid
  ),
...
network_plot_RF <- function(ig) {
  return(
    ggplot(ig) + geom_edge() +
      ...
      coord_equal() +
      scale_color_network(
        values = c(SIR_colors, active = "red", inactive = "grey")
      ) + theme_network()
  )
}
...
network_plot_histograms <- function(s_dt) {
  ...
  durahist_dt <- ref_dt[, .N, keyby = .(duration)]
  sizehist_dt <- ref_dt[, .N, keyby = .(final_size)]
  ...
  geom_dens <- function(
    ...
    geom_bar(fill = fill, stat = stat, width = width, ...)
  }

  # heat map of duration vs final size outcomes
  p_heat <- ggplot(heat_dt) + aes(duration, final_size, fill = N / samples) +
    geom_tile(alpha = 0.7) +
    sx + sy + sdenfill + coord_max() +
    theme_minimal() + theme(
      legend.position = c(0, 0.95), legend.justification = c(0, 1)
    )

  # density plot of durations
  p_dur <- ggplot(durahist_dt) + aes(duration, N / samples) +
    geom_dens() +
    sx + sdeny + coord_max(ymax = max_den) +
    theme_minimal() + theme(
      axis.title.x = element_blank(), axis.text.x = element_blank()
    )

  # density plot of final sizes
  p_sz <- ggplot(sizehist_dt) + aes(N / samples, final_size) +
    geom_dens(orientation = "y") +
    sdenx + sy + coord_max(xmax = max_den) +
    theme_minimal() + theme(axis.title = element_blank())

  # roll them all up
  # TODO replace plot_spacer with some text info?
  p_tot <- p_dur + plot_spacer() + p_heat + p_sz +
